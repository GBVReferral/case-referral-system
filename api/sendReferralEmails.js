...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!referralToOrg || !referralData) {
      res.status(400).json({ error: "Missing referralToOrg or referralData" });
      return;
    }

    // Query Firestore for focal persons in the target organization
    const usersRef = collection(db, "users");
    const q = query(usersRef, where("organization", "==", referralToOrg), where("role", "==", "Focal Person"));
    const querySnapshot = await getDocs(q);

    if (querySnapshot.empty) {
      console.log("No focal persons found for organization:", referralToOrg);
      res.status(404).json({ error: "No focal persons found" });
      return;
    }
    ...
    res.status(200).json({ success: true, message: "Emails sent to focal persons" });
